import { Box ,Text,Image ,Flex,Grid,Heading,Button,GridItem, Select, Stack, Center} from '@chakra-ui/react'
    ...
    useEffect(()=>{
      const brand = searchParams.getAll("brand")
        if( location || soundbar.length === 0){
            ...
            dispatch(FetchSoundBar(queryParams))
        }
     ...
     minHeight="2rem"
      ...
      >
        ...
        >
            ...
            {soundbar.length > 0 && soundbar.map((item)=>{
                return <GridItem 
                key={item.id}
               alignContent="center">
                <Box
